...
if (-not $HostPath) {
  foreach ($ownershipRoot in $ownershipRoots) {
    ...
    if ((Test-Path -LiteralPath $receipt -PathType Leaf) -or (Test-Path -LiteralPath $journal -PathType Leaf) -or (Test-Path -LiteralPath $aclState -PathType Leaf) -or $hasLease) {
...
try {
  ...
  # Retire both generations with the same receipt/SID/filter verification. Never infer ownership
  # from the directory name, and never erase evidence after a cancelled or failed native removal.
  foreach ($ownershipRoot in $ownershipRoots) {
    if (-not (Test-Path -LiteralPath $ownershipRoot)) { continue }
    ...
    if ($removeCode -ne 0) { exit $removeCode }
