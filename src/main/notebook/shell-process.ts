...
const encodePowerShellCommand = (command: string): string => {
  ...
  const script = [
    ...
    '}',
    '$appUtf8 = [System.Text.UTF8Encoding]::new($false)',
    '[Console]::OutputEncoding = $appUtf8',
    '$OutputEncoding = $appUtf8',
    `$appCommandBase64 = '${encodedCommand}'`,
    '$global:LASTEXITCODE = 0',
    "$ProgressPreference = 'SilentlyContinue'",
    "$ErrorActionPreference = 'Stop'",
    'try {',
    '$appCommandText = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($appCommandBase64))',
    '$appCommand = [ScriptBlock]::Create($appCommandText)',
    '& $appCommand',
    '$appSucceeded = $?',
    '$appNativeExitCode = $LASTEXITCODE',
    'if ($appNativeExitCode -is [int] -and $appNativeExitCode -ne 0) { exit $appNativeExitCode }',
    'if ($appSucceeded) { exit 0 }',
    '} catch {',
