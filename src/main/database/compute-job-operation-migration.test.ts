...
describe('Compute Job operation migration', () => {
  it('replays an unledgered operation suffix without rewriting ComputeJob', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open-science-migrations" WHERE "id" IN ('0023_compute_job_operation', '0024_compute_job_file_evidence', '0025_managed_file_version_foundation', '0026_compute_job_remote_cleanup', '0027_project_session_defaults', '0028_database_numeric_and_null_constraints', '0029_compute_host_execution_mode', '0030_literature_foundation', '0031_project_archive_revision', '0032_permission_approval_summary', '0033_compute_job_harvest_retry', '0034_background_result_delivery', '0035_literature_pdf_provenance', '0036_content_verification_observation')`
    )
    ...
    await expect(
      client.$queryRawUnsafe<Array<{ id: string }>>(
        `SELECT "id" FROM "_open-science-migrations" ORDER BY "id" DESC LIMIT 2`
      )
    ).resolves.toEqual([
