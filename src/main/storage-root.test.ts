...
describe('computeDefaultDataRoot', () => {
  ...
  it('discovers the released default until its verified migration is committed', async () => {
    const previous = join(homeDir, 'OpenScience')
    await mkdir(join(previous, 'notebooks'), { recursive: true })
    expect(computeDefaultDataRoot()).toBe(previous)
    const current = join(homeDir, 'Open-Science')
    await mkdir(current)
    expect(computeDefaultDataRoot()).toBe(previous)
    initDataRoot(current)
    expect(computeDefaultDataRoot()).toBe(current)
  })
