...
describe('migration reference safety', () => {
  it('requires verified complete runtime preservation before committing a brand migration', async () => {
    const extra = join('runtime', 'envs', 'custom', 'pip-extra.txt')
    await mkdir(dirname(join(currentDataRoot, extra)), { recursive: true })
    await writeFile(join(currentDataRoot, extra), 'installed extra package')
    const deps = {
      ...fakeDeps(),
      validateProvenanceState: async () => {},
      preserveRuntimeTree: async (_from: string, to: string) => {
        await mkdir(dirname(join(to, extra)), { recursive: true })
        await writeFile(join(to, extra), 'installed extra package')
      }
    }
    ...
    expect(marker?.runtimeCopyInventory).toBeDefined()
    expect(await readFile(join(target, extra), 'utf8')).toBe('installed extra package')
    await writeFile(join(target, extra), 'tampered runtime')
    expect(
      await commitDataRootSwitch({ ...deps, expectedToken: marker!.token }, emptyParent)
    ).toMatchObject({ ok: false })
    expect(deps.setDataRoot).not.toHaveBeenCalled()
    expect(await readFile(join(currentDataRoot, extra), 'utf8')).toBe('installed extra package')
  })
  it('aborts brand migration when complete runtime preservation fails', async () => {
    const deps = {
      ...fakeDeps(),
      validateProvenanceState: async () => {},
      preserveRuntimeTree: async () => {
        throw new Error('unrelocatable native package')
      }
    }
    expect(await runDataRootMigration(deps, emptyParent, runOpts())).toMatchObject({ ok: false })
    expect(deps.setDataRoot).not.toHaveBeenCalled()
