...
> 💡 **[AIPOCH Open-Science v0.27.0 已發佈](https://github.com/aipoch/open-science/releases/latest)** _（最後更新於 2026 年 9 月）_。AIPOCH Open-Science v0.27.0 擴展文獻工作區，並讓長時間工作改在背景執行：一次匯入大量 PDF，附逐檔進度與重試；Notebook 與命令列工作可在背景執行並自動交付結果；另有一輪涵蓋跨用戶端同步、資料位置遷移與匯入完整性的廣泛文獻穩定性強化。核心應用程式技能一律保持啟用，無介面 CLI 與 Task SDK 新增連接器管理，Mermaid 圖表渲染更順暢，無介面 Linux 部署也新增明確的憑證檔案儲存模式。詳情請參閱[最新版本說明](https://github.com/aipoch/open-science/releases/latest)。
...
| **Python、R、Notebook 與 HPC** | 執行持久化 Python、R 和 REPL 核心以及納入同一歷史記錄的命令列指令，可使用受管理離線環境或自備直譯器。在本機工作，或透過 SSH 連接遠端主機並使用 Slurm 向 HPC 叢集提交 Notebook 工作；受保護的網路存取、加密憑證、套件與變數檢視、共用終端和漸進式歷史載入讓運算過程可控且可觀察。長時間的 Notebook、REPL 與命令列工作可在背景執行——在釋放智能體輪次的同時保留確切的執行識別、取消與溯源，並在本機執行與遠端運算作業之間自動交付結果。外部 R 環境的套件管理仍需手動完成。      |
...
| **本機資料、隱私、權限與驗證** | 將專案資料、應用程式狀態和 Notebook 快取保存在可設定、可移轉的本機儲存空間中；支援系統、手動或直接連線 Proxy 模式，以及包含 30 天活動熱圖和逐次執行歸因的詞元儀表板。透過 `Ask for approval`、`Auto-approve edits` 或 `Full access`、限定範圍的授權、集中式憑證（無介面 Linux 部署可使用明確的檔案儲存模式）、使用者核准的運算網域以及連接器和工具層級原則控制操作。可選審查者會稽核對話記錄、執行記錄檔和產物，回報通過/警告/失敗結果，並可執行保留持久證據的有界修正迴圈。 |
