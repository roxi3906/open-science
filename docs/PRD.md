...
- **macOS unsigned builds.** Without an Apple Developer ID, packaged builds are deep ad-hoc signed at pack time (see `build/adhoc-sign.cjs`) so Gatekeeper shows the bypassable "unidentified developer" prompt instead of an unrecoverable "app is damaged" error on a quarantined copy. Users installing a self-built or community-distributed `.app` need to either right-click → Open, or clear the quarantine flag directly — see [README: Building From Source](../README.md#building-from-source-macos-gatekeeper-note) for the exact command.
